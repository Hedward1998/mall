...
function initialCookie() {
    ...
    if(document.cookie.length>0) {
        ...
        if(url !== null) {
            ...
            $("#div_background").css("background-image", "url(/mall/res/images/admin/loginPage/background-4.jpg)");
        }
        if(username !== null){
        ...
        $.getJSON("/mall/admin/login/profile_picture",{"username":username},function (data) {
            if(data.success){
                if(data.srcString !== null){
                    $("#img_profile_picture").attr("src", "/mall/res/images/item/adminProfilePicture/" + data.srcString);
                    return true;
                }
            }
        });
    }
    $("#img_profile_picture").attr("src","/mall/res/images/admin/loginPage/default_profile_picture-128x128.png");
}