<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<html>
<head>
    <script>
        //检索数据集
        var dataList = {
            "news_title": null,
            "news_status_array": null,
            "orderBy": null,
            "isDesc": true
        };
        $(function () {
            /******
             * event
             ******/
            //点击查询按钮时
            $("#btn_news_submit").click(function () {
                var news_title = $.trim($("#input_news_title").val());
                //状态数组
                var status_array = [];
                $(".radio_status:checked").each(function () {
                    status_array.push($(this).val());
                });
                //封装数据
                dataList.news_title = encodeURI(news_title);
                dataList.news_status_array = status_array;

                getData($(this), "admin/news/0/10", dataList);
            });
            //点击刷新按钮时
            $("#btn_news_refresh").click(function () {
                //清除数据
                dataList.news_title = null;
                dataList.news_status_array = null;
                dataList.orderBy = null;
                dataList.isDesc = true;
                //获取数据
                getData($(this), "admin/news/0/10", null);
                //清除排序样式
                var table = $("#table_news_list");
                table.find("span.orderByDesc,span.orderByAsc").css("opacity","0");
                table.find("th.data_info").attr("data-sort","asc");
            });
            //点击th排序时
            // $("th.data_info").click(function () {
            //     var table = $("#table_news_list");
            //     if(table.find(">tbody>tr").length <= 1){
            //         return;
            //     }
            //     //获取排序字段
            //     dataList.orderBy = $(this).attr("data-name");
            //     //是否倒序排序
            //     dataList.isDesc = $(this).attr("data-sort")==="asc";
            //
            //     getData($(this), "admin/news/0/10", dataList);
            //     //设置排序
            //     table.find("span.orderByDesc,span.orderByAsc").css("opacity","0");
            //     if(dataList.isDesc){
            //         $(this).attr("data-sort","desc").children(".orderByAsc.orderBySelect").removeClass("orderBySelect").css("opacity","1");
            //         $(this).children(".orderByDesc").addClass("orderBySelect").css("opacity","1");
            //     } else {
            //         $(this).attr("data-sort","asc").children(".orderByDesc.orderBySelect").removeClass("orderBySelect").css("opacity","1");
            //         $(this).children(".orderByAsc").addClass("orderBySelect").css("opacity","1");
            //     }
            // });
            //点击table中的数据时
            $("#table_news_list").find(">tbody>tr").click(function () {
                trDataStyle($(this));
            });
        });
        //获取新闻公告数据
        function getData(object,url,dataObject) {
            var table = $("#table_news_list");
            var tbody = table.children("tbody").first();
            $.ajax({
                url: url,
                type: "get",
                data: dataObject,
                traditional: true,
                success: function (data) {
                    //清空原有数据
                    tbody.empty();
                    //设置样式
                    $(".loader").css("display","none");
                    object.attr("disabled",false);
                    //显示新闻公告统计数据
                    $("#news_count_data").text(data.newsCount);
                    if (data.newsList.length > 0) {
                        for (var i = 0; i < data.newsList.length; i++) {
                            var status;
                            if (data.newsList[i].news_status === 1) {
                                status = "已发布";
                            } else {
                                status = "未发布";
                            }
                            var news_id = data.newsList[i].news_id;
                            var news_title = data.newsList[i].news_title;
                            var news_publish_person_name = data.newsList[i].news_publish_person_name;
                            var news_publish_date = data.newsList[i].news_publish_date;
                            //显示用户数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' id='cbx_news_select_" + news_id + "'><label for='cbx_news_select_" + news_id + "'></label></td><td title='" + news_title + "'>" + news_title + "</td><td title='\" + news_publish_date + \"'>" + news_publish_date + "</td><td title='" + news_publish_person_name + "'>" + news_publish_person_name + "</td><td title='" + status + "'>" + status + "</td><td><span class='td_special' title='查看新闻公告详情'><a href='javascript:void(0);' onclick='getChildPage(this)'>详情</a></span></td><td hidden  class='news_id'>" + news_id + "</td></tr>");
                        }
                        //绑定事件
                        tbody.children("tr").click(function () {
                            trDataStyle($(this));
                        });
                        //分页
                        var pageUtil = {
                            index: data.pageUtil.index,
                            count: data.pageUtil.count,
                            total: data.pageUtil.total,
                            totalPage: data.totalPage
                        };
                        createPageDiv($(".loader"), pageUtil);
                    }
                },
                beforeSend: function () {
                    $(".loader").css("display","block");
                    object.attr("disabled",true);
                },
                error: function () {

                }
            });
        }

        //获取用户子界面
        function getChildPage(obj) {
            //设置样式
            $("#div_home_title").children("span").text("新闻公告详情");
            document.title = "Mall管理后台 - 新闻公告详情";
            //ajax请求页面
            ajaxUtil.getPage("news/" + $(obj).parents("tr").find(".news_id").text(), null, true);
        }

        //获取页码数据
        function getPage(index) {
            getData($(this), "admin/news/" + index + "/10", dataList);
        }
    </script>
    <style rel="stylesheet">
        #lbl_news_title,#lbl_news_status{
            width: 70px;
        }
    </style>
</head>
<body>
<div class="frm_div text_info">
    <div class="frm_group">
        <label class="frm_label" id="lbl_news_title" for="input_news_title">新闻公告标题</label>
        <input class="frm_input" id="input_news_title" type="text" maxlength="50"/>
        <input class="frm_btn" id="btn_news_submit" type="button" value="查询"/>
        <input class="frm_btn frm_clear" id="btn_clear" type="button" value="重置"/>
    </div>
    <div class="frm_group">
        <label class="frm_label" id="lbl_news_status" for="checkbox_news_status_publish">状态</label>
        <input class="frm_radio radio_status" id="checkbox_news_status_publish" name="checkbox_news_status" type="checkbox" value="1" checked>
        <label class="frm_label" id="lbl_news_status_publish" for="checkbox_news_status_publish">已发布</label>
        <input class="frm_radio radio_status" id="checkbox_news_status_no_publish" name="checkbox_news_status" type="checkbox" value="0" checked>
        <label class="frm_label" id="lbl_news_status_no_publish" for="checkbox_news_status_no_publish">未发布</label>
    </div>
    <div class="frm_group_last">
        <input class="frm_btn frm_refresh" id="btn_news_refresh" type="button" value="刷新新闻公告列表"/>
    </div>
</div>
<div class="data_count_div text_info">
    <svg class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2522" width="16"
         height="16">
        <path d="M401.976676 735.74897c-88.721671 0-172.124196-34.635845-234.843656-97.526197-62.724577-62.86784-97.271394-146.453537-97.271394-235.358379s34.546817-172.490539 97.276511-235.361449c62.715367-62.887282 146.117892-97.522104 234.838539-97.522104 88.719624 0 172.135452 34.633798 234.881518 97.522104 62.704111 62.875003 97.235578 146.4607 97.235578 235.361449 0 88.901773-34.530444 172.487469-97.231485 235.358379C574.112128 701.116195 490.6963 735.74897 401.976676 735.74897zM401.976676 121.204479c-75.012438 0-145.533584 29.290093-198.572568 82.474386-109.585861 109.834524-109.585861 288.539602-0.004093 398.36901 53.043077 53.188386 123.564223 82.47848 198.577684 82.47848 75.015507 0 145.553027-29.291117 198.620663-82.47848C710.126918 492.220514 710.126918 313.511343 600.593246 203.678866 547.530726 150.496619 476.992183 121.204479 401.976676 121.204479z"
              p-id="2523" fill="#FF7874">
        </path>
        <path d="M932.538427 958.228017c-6.565533 0-13.129019-2.508123-18.132986-7.52437L606.670661 642.206504c-9.989515-10.014074-9.969049-26.231431 0.045025-36.220946s26.230408-9.969049 36.220946 0.045025l307.73478 308.497143c9.989515 10.014074 9.969049 26.231431-0.045025 36.220946C945.627537 955.735244 939.081447 958.228017 932.538427 958.228017z"
              p-id="2524" fill="#FF7874">
        </path>
    </svg>
    <span class="data_count_title">查看合计</span>
    <span>新闻公告总数:</span>
    <span class="data_count_value" id="news_count_data">${requestScope.newsCount}</span>
    <span class="data_count_unit">条</span>
</div>
<div class="table_normal_div">
    <table class="table_normal" id="table_news_list">
        <thead class="text_info">
        <tr>
            <th><input type="checkbox" class="cbx_select" id="cbx_select_all"><label for="cbx_select_all"></label></th>
            <th class="data_info" data-sort="asc" data-name="news_title">
                <span>新闻公告标题</span>
                <%--<span class="orderByDesc"></span>--%>
                <%--<span class="orderByAsc orderBySelect"></span>--%>
            </th>
            <th class="data_info" data-sort="asc" data-name="news_publish_date">
                <span>发布时间</span>
                <%--<span class="orderByDesc"></span>--%>
                <%--<span class="orderByAsc orderBySelect"></span>--%>
            </th>
            <th class="data_info" data-sort="asc" data-name="news_publish_person_name">
                <span>发布人</span>
                <%--<span class="orderByDesc"></span>--%>
                <%--<span class="orderByAsc orderBySelect"></span>--%>
            </th>
            <th class="data_info" data-sort="asc" data-name="news_status">
                <span>状态</span>
                <%--<span class="orderByDesc"></span>--%>
                <%--<span class="orderByAsc orderBySelect"></span>--%>
            </th>
            <th>操作</th>
            <th hidden>新闻公告ID</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${requestScope.newsList}" var="news">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_news_select_${news.news_id}"><label for="cbx_news_select_${news.news_id}"></label></td>
                <td title="${news.news_title}">${news.news_title}</td>
                <td title="${news.news_publish_date}">${news.news_publish_date}</td>
                <td title="${news.news_publish_person_name}">${news.news_publish_person_name}</td>
                <td>
                    <c:choose>
                        <c:when test="${news.news_status==1}">已发布</c:when>
                        <c:otherwise>未发布</c:otherwise>
                    </c:choose>
                </td>
                <td><span class="td_special" title="查看新闻公告详情"><a href='javascript:void(0)'
                                                               onclick='getChildPage(this)'>详情</a></span></td>
                <td hidden class="news_id">${news.news_id}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    <%@ include file="include/page.jsp" %>
    <div class="loader"></div>
</div>
</body>
</html>
