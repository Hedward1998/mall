    ...
    <script>
        ...
        $(function () {
            ...
            $("#btn_admin_submit").click(function () {
                ...
                getData($(this), "admin/admin/0/10", dataList);
            });
            //点击刷新按钮时
            $("#btn_admin_refresh").click(function () {
                ...
                //获取数据
                getData($(this), "admin/admin/0/10", null);
                //清除排序样式
                ...
                success: function (data) {
                    ...
                    if (data.adminList.length > 0) {
                        for (var i = 0; i < data.adminList.length; i++) {
                            ...
                            //显示管理员数据
                            tbody.append("<tr><td><input type='checkbox' class='cbx_select' id='cbx_admin_select_" + admin_id + "'><label for='cbx_admin_select_" + admin_id + "'></label></td><td title='" + admin_name + "'>" + admin_name + "</td><td title='" + admin_nickname + "'>" + admin_nickname + "</td><td><span class='td_special' title='查看用户详情'><a href='javascript:void(0);' onclick='getChildPage(this)'>详情</a></span></td><td hidden  class='admin_id'>" + admin_id + "</td></tr>");
                        }
                        //绑定事件
        ...
        function getChildPage(obj) {
            ...
            if (obj === null) {
                ...
                title = "管理员详情";
                url = "admin/"+$(obj).parents("tr").find(".admin_id").text();
            }
            //设置样式
...
<div class="frm_div text_info">
    ...
    <div class="frm_group_last">
        <input class="frm_btn frm_add" id="btn_admin_add" type="button" value="添加管理员" onclick="getChildPage(null)"/>
...
<div class="table_normal_div">
    <table class="table_normal" id="table_admin_list">
        ...
        <tr>
            ...
            <th class="data_info" data-sort="asc" data-name="admin_name">
                <span>管理员姓名</span>
                <span class="orderByDesc"></span>
                <span class="orderByAsc orderBySelect"></span>
            </th>
            <th class="data_info" data-sort="asc" data-name="admin_nickname">
                <span>管理员昵称</span>
                <span class="orderByDesc"></span>
                <span class="orderByAsc orderBySelect"></span>
            </th>
        ...
        <c:forEach items="${requestScope.adminList}" var="admin">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_admin_select_${admin.admin_id}"><label for="cbx_admin_select_${admin.admin_id}"></label></td>
                <td title="${admin.admin_name}">${admin.admin_name}</td>
